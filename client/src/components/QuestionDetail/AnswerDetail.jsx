    ...
    const addAnswer = async () => {
        try {
            if (textInput) {
                ...
                const updatedAnswers = [...customQuestion.answers, {
    ...
    return (
        <div>
                        ...
                        return <div className=' question__content md:w-[80%] m-auto mt-10 p-5 rounded md flex text-white'>
                            <div className="answer__left ">
                                ...
                                {customQuestion?.answers?.find(x => x?._id == item?._id).correct.find(x => x == userid) ?
                                    ...
                                    <button className='p-2 bg-white text-gray-600 rounded-full' onClick={() => answerCorrect(item?._id)}>
                                        <AiOutlineCaretUp />
                                    </button>
                                }
            ...
            <div className="add__answer__content md:w-[80%] m-auto mt-16 ">
                <textarea name="" id="" cols="30" rows="10" className='w-full resize-none p-5 rounded-md' value={textInput} placeholder='Answer ...' onChange={(e) => { setTextInput(e.target.value) }}></textarea>
                <div className='w-full flex justify-end'>
                    <button onClick={() => { addAnswer(), setTextInput("") }} className='px-3 py-2 bg-purple-700 text-white rounded-md my-3'>Add Answer</button>
                </div>
