...
function AllQuestionDetail() {
    ...
    const addQuestion = async () => {
        try {
            const response = await axios.post(`http://localhost:8080/question/user/${userid}`, { text: inputText });
    ...
    return (
        <div>
            <div className="allquestion__header flex justify-between items-center">
                ...
                {
                    <button className='text-white px-3 py-2 bg-slate-600 rounded-md' onClick={() => setOpenModal(!openModal)}>
            ...
            {
                openModal && <div className="create__question__modal h-[80%] md:w-[50%] md:h-[60%] p-5 m-auto absolute inset-0 rounded-md">
                    <textarea name="" id="" cols="30" rows="10" className='w-full resize-none rounded-md p-3 text-lg' onChange={(e) => setInputText(e.target.value)}></textarea>
                    <button className='px-3 py-2 bg-purple-600 text-white rounded-md mt-3' onClick={() => addQuestion()}>Create</button>
                </div>
            }
            ...
            <div className='my-16'>
                {
                    allQuestion?.map(item => {
                        return (
                            <div onClick={() => goToQuestion(item?._id)} className='block question__list w-[90%] m-auto  p-5  my-5 rounded-md'>
