...
function QuestionDetailComp() {
  ...
  const deleteQuestion = async (questionId) => {
    try {
      ...
      if (response.status === 200) {
        const updatedQuestion = allQuestion?.filter(
  ...
  return (
    <div className="min-h-screen p-5">
      <div
        onClick={() => goBack()}
        className="p-2 question__content rounded-full text-white text-xl my-5 inline-block"
      >
        <BiArrowBack />
      </div>
          ...
          {editActive ? (
            <div className="w-full">
              ...
              <button
                ...
                onClick={() => {
                  editQuestion(customQuestion?._id), setEditActive("");
                }}
              >
                save
              </button>
            ...
            dangerouslySetInnerHTML={{ __html: customQuestion?.text }}
          />
