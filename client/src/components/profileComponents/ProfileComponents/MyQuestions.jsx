      ...
      <div className='flex  gap-y-9 flex-wrap cursor-pointer '>
        <h1 className='text-white text-xl'>My Questions</h1>
        {
          data?.questions?.map(item => {
            return <div onClick={() => goToQuestion(item._id)} className='w-full question__content drop-shadow-2xl shadow-white border rounded-md p-5  text-white justify-between flex'>
              <p>{item.text}</p>
