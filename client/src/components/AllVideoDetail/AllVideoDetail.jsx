...
function AllVideoDetail() {
  ...
  const searchVideoFunc = () => {
    const filteredVideos = allVideo.filter((x) =>
  ...
  const applyFilters = (videos) => {
    ...
    if (selectedTimeFilter === "lastHour") {
      filteredVideos = filteredVideos.filter((item) =>
        moment(item.uploadDate).isAfter(moment().subtract(1, "hour"))
      );
    } else if (selectedTimeFilter === "thisDay") {
      filteredVideos = filteredVideos.filter((item) =>
        moment(item.uploadDate).isAfter(moment().subtract(1, "day"))
      );
    } else if (selectedTimeFilter === "thisWeek") {
      filteredVideos = filteredVideos.filter((item) =>
        moment(item.uploadDate).isAfter(moment().subtract(1, "week"))
      );
    } else if (selectedTimeFilter === "thisMonth") {
      filteredVideos = filteredVideos.filter((item) =>
        moment(item.uploadDate).isAfter(moment().subtract(1, "month"))
      );
    } else if (selectedTimeFilter === "thisYear") {
      filteredVideos = filteredVideos.filter((item) =>
        moment(item.uploadDate).isAfter(moment().subtract(1, "year"))
      );
    }

    if (selectedCategoryFilter !== "") {
      filteredVideos = filteredVideos.filter(
        (item) => item.categoryId?.name === selectedCategoryFilter
      );
    }
    ...
    <div className="p-5">
      <div className="filter__videos flex w-full">
        <div className="search__content flex md:w-[50%] m-auto">
          ...
          />
          <button
            className="px-3 py-2 text-white bg-blue-500 rounded-md ml-3"
            onClick={() => searchVideoFunc()}
          >
            Search
          </button>
        </div>
        <button
          className="p-2 rounded-md flex gap-2 items-center bg-blue-400 text-white ml-3"
      ...
      <div className="grid grid-cols-4 gap-x-5  gap-y-9 flex-wrap  my-16">
        {searchVideo?.map((item) => {
          return (
            <div
              onClick={() => goToVideo(item._id)}
              className="user__videos hover:scale-[1.05] transition-all h-[320px] relative shadow-lg rounded-md w-full"
            >
              <div className="video__cover__img shadow-lg ">
                <img
                  ...
                  className="bg-slate-400 h-[200px] w-full  object-cover rounded-md"
                />
              </div>
      ...
      {openModal && (
        <div className="filter__modal   max-w-max max-h-max bg-white top-[8%] left-[5%]  md:top-[20%] md:left-[40%] absolute inset-0 p-5 ">
          <div className="flex justify-between">
            ...
            <button
              onClick={() => {
                resetFilters(), setOpenModal(false);
              }}
            >
              <BiReset className="text-2xl hover:-rotate-180 transition-transform" />
            </button>
          </div>
          <div className="  flex ">
            <div className="filter__time p-5">
              ...
              <ul className="flex  flex-col gap-3">
                <li
                  className={`hover:opacity-80 hover:scale-105 cursor-pointer ${
                    selectedTimeFilter === "lastHour"
