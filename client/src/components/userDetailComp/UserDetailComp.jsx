...
import nodata from "../../img/No data (2).gif";
...
function UserDetailComp() {
  ...
  const followUser = async (paramsValue) => {
    ...
    try {
      await axios.post(
        `http://localhost:8080/user/follow/${user?._id}/followUserId/${paramsId}`
      );
      const updateUser = { ...user, follow: [...user?.follow, paramsValue] };
      let updateUserid = {
        ...customUser,
        followers: [...customUser?.followers, paramsId],
      };
  ...
  const unfollow = async (paramsId) => {
    ...
    try {
      ...
      const updateUser = {
        ...user,
        follow: [...user?.follow?.filter((x) => x._id !== paramsId)],
      };
      let updateUserid = {
        ...customUser,
        followers: [...customUser?.followers?.filter((x) => x !== userid)],
      };
    ...
    <div>
      <div className="user__header__section md:flex w-full justify-between ">
        <div className="user__img lg:w-[25%] md:w-[30%]">
          {customUser?.image ? (
            <img
              src={`http://localhost:8080/accountimg/images/${customUser?.image}`}
              className={` h-[300px] lg:h-[250px] md:h-[200px] md:w-[200px] w-full lg:w-[250px] object-fill rounded-full  bg-slate-500 ${
                customUser?.image ? "p-0" : "p-1"
              }`}
            />
          ) : (
            <img
              src={profileImg}
              className={` h-[230px] w-full md:w-[230px] object-fill rounded-lg  bg-slate-500  `}
            />
          )}
        </div>
        <div className="user__info__content  md:w-[70%]">
          <div className="title__links  md:flex justify-between items-center">
            ...
            <ul className="social__links flex gap-5">
              {customUser?.social?.map((item, index) => (
                <li
                  key={index}
                  className="h-[30px] w-[30px] md:h-[25px] md:w-[25px]  block relative hover:-top-1"
                >
                  <a
                    href={item.link}
                    target="_blank"
                  ...
                  >
                    <img
                      ...
                      alt={item.name}
                      className="social__icon w-full h-full"
                    />
                  </a>
                </li>
              ))}
          ...
          <div className="user__followers__following md:hidden block my-5 ">
            <div className="flex">
              <p className="flex flex-col justify-center items-center w-full px-10 py-2 rounded-l-md bg-blue-400 text-md text-white border">
                <span className="font-bold">
                  {numeral(customUser?.follow?.length).format("0,0a")}
                </span>
                following
              </p>
              <p className="flex flex-col justify-center items-center w-full px-10 py-2 bg-blue-400 text-md text-white border border-l-0 rounded-r-md">
                <span className="font-bold">
                  {numeral(customUser?.followers?.length).format("0,0a")}
                </span>
                followers
              </p>
            </div>

            {customUser?._id !== userid ? (
              userid &&
              typeof customUser?.followers?.find((item) => item == userid) ==
                "string" ? (
                <button
                  className="my-3 w-full bg-blue-500 px-16 py-2 text-xl text-white hover:bg-blue-600 rounded-md border "
                  onClick={() => {
                    unfollow(customUser._id);
                  }}
                >
                  Unfollow
                </button>
          ...
          <div className="user__followers__following md:flex hidden">
            <p className="flex flex-col justify-center items-center px-10 py-2 rounded-l-md bg-blue-400 text-md text-white border">
              <span className="font-bold">
                {numeral(customUser?.follow?.length).format("0,0a")}
              </span>
              following
            </p>
            <p className="flex flex-col justify-center items-center px-10 py-2 bg-blue-400 text-md text-white border border-l-0">
              <span className="font-bold">
                {numeral(customUser?.followers?.length).format("0,0a")}
              </span>
              followers
            ...
            {customUser?._id !== userid ? (
              ...
              typeof customUser?.followers?.find((item) => item == userid) ==
                "string" ? (
                <button
                  className="bg-blue-500 px-16 py-2 text-xl text-white hover:bg-blue-600 rounded-r-md border border-l-0"
                  onClick={() => {
                    unfollow(customUser._id);
                  }}
                >
                  ...
                  <div className="video__info__content  p-3">
                    ...
                    <div className="flex justify-end items-center absolute bottom-0 right-3 w-full">
                      <p className="text-white">
                        {" "}
                        {moment(item?.uploadDate).startOf("minute").fromNow()}
                      </p>
                    </div>
                  </div>
                </div>
