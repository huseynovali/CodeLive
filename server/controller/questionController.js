...
const questionController = {
    getQuestionsByVideoId: async (req, res) => {
        ...
        try {
          const questions = await Question.find({ video: videoId }).populate("author", "username");
  ...
  editQuestion: async (req, res) => {
    ...
    try {
      ...
      const user = await User.findById(question.author);
      if (user) {
        const questionIndex = user.questions.indexOf(questionId);
        if (questionIndex !== -1) {
          user.questions[questionIndex] = question._id;
          await user.save();
        }
      }
